...
// 'image' prop yahan add kiya gaya hai
const ProductCard = ({ name, desc, price, type, prices, addon, image }) => {
  ...
  const handleConfirm = (orderItem) => {
    addToCart({
      id: name,
      name: name,
      price: displayPrice,
      image: image, // Image ab cart mein pass hogi
  ...
  return (
    <>
      <motion.div
        whileHover={{ y: -6 }}
        // Transition fast kar di (stiffness 300 -> 500)
      ...
      >
        ...
        <div className="w-full h-28 sm:h-36 md:h-40 rounded-[12px] sm:rounded-[14px] mb-2.5 overflow-hidden relative">
          <motion.div
            ...
            transition={{ duration: 0.3 }} // Fast zoom
            className="absolute inset-0 bg-zinc-100"
          >
            {/* Actual image render ho rahi hai */}
            <img 
              src={image} 
              alt={name} 
        ...
        <div className="mt-auto">
          {type === 'multi-price' && (
            <div className="flex bg-zinc-100 p-0.5 rounded-full text-[9px] sm:text-[10px] font-black mb-2 relative">
              <button
                ...
                className={`flex-1 py-1 rounded-full z-10 transition-colors duration-200 ${!isDouble ? 'text-white' : 'text-zinc-400'}`}
              >
                SINGLE
              </button>
              <button
          ...
          {addon && (
            <label className="flex items-center gap-1.5 cursor-pointer mb-2">
              <div
                ...
                className="relative w-3.5 h-3.5 sm:w-4 sm:h-4 rounded flex items-center justify-center border transition-all duration-100"
              >
                <motion.div
                  animate={{
          ...
          <div className="flex justify-between items-center mt-1">
            ...
            <motion.button
              ...
              className="w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center bg-[#1a1611] text-white hover:bg-[#C5A267] transition-colors duration-150"
            >
